...
Worker.prototype.createProcess = function() {
    ...
    var clArgs = [__dirname + '/../../lib/worker/Worker.js', this.id, this.pool.workerFile, this.pool.restartSlimerAfter];
    if (this.pool.slimerjsOptions) {
        clArgs.unshift.apply(clArgs, this.pool.slimerjsOptions);
    }

    // Spawn process
    this.proc = cp.spawn(that.pool.slimersBinary, clArgs, { cwd : process.cwd() });
    this.proc.on('error', function (err) {
        if (err.message.indexOf('ENOENT') !== -1) {
            throw new Error('slimersBinary not found: ' + that.pool.slimersBinary + ' (Full error: ' + err.message + ')');
        } else {
    ...
    this.proc.on('close', function (code, signal) {
        ...
        if (signal !== 'SIGTERM') {
            ...
            if (that.alive) {
                log(that.id, 'recreating slimerjs instance');
                that.createProcess();
            }
        }
...
Worker.prototype.work = function(data, givenJobCallback) {
    ...
    // we will now send this job the the slimerJS instance via REST
    // the slimerJS instance has a port opened for this which accepts REST calls
